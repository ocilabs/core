...
=== Introduction
The ocloud framework is a collection of automation scripts and aims at operators launching services on Oracle Cloud Infrastructure (OCI). It acclerates the process of setting up a foundation for enterprise services. Even though provisioning a cloud server has become a matter of pressing a button, building a hosting platform for intra- and extranet services on third party infrastructure remains challenging for most IT operators. Using this framework system administrator employ on link:https://en.wikipedia.org/wiki/Infrastructure_as_code["Infrastructure as Code" (IaC)] to provision dedicated resources including the necessary controls to address typical compliance and security requirements. 
