...
=== Introduction
The ocloud framework is a collection of automation scripts that acclerate the deployment process for cloud services on link:https://www.oracle.com/cloud/[Oracle Cloud Infrastructure (OCI)]. Provisioning a cloud server has become a matter of pressing a button, but enterprise user need more than a server to build a hosting platform for intra- and extranet services. This framework aims at operators and acclerates the process to build a private infrastructure platform. Utilizing  link:https://en.wikipedia.org/wiki/Infrastructure_as_code["Infrastructure as Code" (IaC)] IT departments become more agile. Preconfigured deployment templates for dedicated infrastructure include operator controls that address typical compliance and security requirements and preserve the flexibility to deploy services on-demand with automation scripts that are exposed through a REST API publish through the link:https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Concepts/resourcemanager.htm[Oracle's Resource Manager (ORM)]. 
