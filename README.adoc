...
=== Introduction
The ocloud framework is a collection of automation scripts that acclerate the deployment process for cloud services on Oracle Cloud Infrastructure (OCI). System administrators benefit from pre-configured resources launching a hosting platform for intra- and extranet services. Even though provisioning a cloud server has become a matter of pressing a button, building a platform to host enterprise services is still challenging. Using this framework operators employ on link:https://en.wikipedia.org/wiki/Infrastructure_as_code["Infrastructure as Code" (IaC)] to provision dedicated resources including the necessary controls that address typical compliance and security requirements of entterprise IT departments. 
