...
=== Introduction
The ocloud framework is a collection of automation scripts that acclerate the deployment process for cloud services on link:https://www.oracle.com/cloud/[Oracle Cloud Infrastructure (OCI)]. Provisioning a cloud server has become a matter of pressing a button, but what serves developers well is not necessarily optimal for operators. Businesses need resources to build hosting platforms for intra- and extranet services, fast and cost efficient. This framework helps operators to become more agile and adopt cloud services without giving up a tight control over the infrastrucutre. Provisioning templates for dedicated infrastructure include operator controls to address compliance and security requirements. Relying on link:https://en.wikipedia.org/wiki/Infrastructure_as_code["Infrastructure as Code" (IaC)] preserves the flexibility to deploy services on-demand . The link:https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Concepts/resourcemanager.htm[Oracle's Resource Manager (ORM)] allows to expose automation scripts through REST API and build an own private marketplace for cloud services. 
