...
=== Introduction
The OCloud framework helps cloud engineers to build services with Oracle Cloud Infrastructure (OCI). While provisioning a cloud server has become a matter of pressing a button, launching a hosting platform for intra- and extranet services is more challenging. This framework aims at system administrators and operators, new to OCI to acclerate the onboarding process for enterprise services. link:https://en.wikipedia.org/wiki/Infrastructure_as_code["Infrastructure as Code" (IaC)] is employed to provision dedicated infrastructure and enable the necessary operator controls to address typical compliance and security requirements. 
