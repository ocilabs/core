...
=== Introduction
The ocloud framework is a collection of automation scripts that acclerate the deployment process for cloud services on link:https://www.oracle.com/cloud/[Oracle Cloud Infrastructure (OCI)]. This framework helps operators to adopt cloud automation without without compromising on control. While provisioning a cloud server has become a matter of pressing a button, launching a hosting platforms for intra- and extranet services requires operator controls that ensure the redemption of security and compliance regulations on system level. Provisioning templates for dedicated resources include operator controls to address compliance and security requirements. Relying on link:https://en.wikipedia.org/wiki/Infrastructure_as_code["Infrastructure as Code" (IaC)] preserves the flexibility to deploy services on-demand . The link:https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Concepts/resourcemanager.htm[Oracle's Resource Manager (ORM)] allows to expose automation scripts through REST API and build an own private marketplace for cloud services. 
